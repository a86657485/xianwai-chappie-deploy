...
2. 用已有静态服务器运行此文件夹；安装了Python的电脑也可在此文件夹运行：
   ...
   Windows可将 `python3` 换为 `py`。若4174已在使用，请同步修改 `server.py` 中的端口，并在浏览器地址中使用相同端口。
3. 打开 `http://127.0.0.1:4174/`，先显示黑色发布会开场，扫描结束后点击“进入汇报”。

上传到“作品相册”的图片和视频会保存到当前文件夹的 `uploads/` 目录；同一局域网内的设备刷新页面即可看到最新素材。请保持 `server.py` 持续运行，不要改用只读的 `python3 -m http.server`。
